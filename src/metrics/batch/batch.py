...
def confidence_score(
    ...
    min_transfer_total_erc20: int=src.options.MIN_TRANSFER_TOTAL_ERC20,
    min_transfer_total_native: int=src.options.MIN_TRANSFER_TOTAL_NATIVE,
    max_batching_fee: int=src.options.MAX_BATCHING_FEE[1]
) -> float:
    """Evaluate the probability that multiple transfers were bundled in a transaction."""
    _scores = []
    # parse the log
    ...
    _scores.append(src.metrics.probabilities.indicator_to_probability(
        indicator=src.indicators.batch.input_data_has_batching_selector(_data),
    ...
    _scores.append(src.metrics.probabilities.indicator_to_probability(
        indicator=src.indicators.batch.input_data_has_matching_arrays_of_values_and_addresses(data=_data, min_length=min_transfer_count),
